...
class MarcGrok {
   ...
   */
  private function extractNames(string $tag): array {
    $names = [];
    ...
    foreach ($this->record->query($tag) as $field) {
      ...
      // Use "Titles and words associated with a name" as the suffix in
      // CSL, if available.
   ...
   * Memoized.
   *
   * Most info comes from field 773 - Host Item Entry (R).
   *
   * @return array
   *   Associative array of container info.
   */
   ...
   * Memoized.
   *
   * Info comes from fields 260 and 264.
   *
   * @return array
   ...
   * subfields here. Simply use the first subfield if available.
   *
   * @return array
   *   Associative array of publication info.
   */
